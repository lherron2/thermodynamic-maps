...
class Backbone(nn.Module):
    ...
    def __init__(
        ...
        print("Using device ",self.device,"for TM",flush=True)
    ...
    def load_state(self, model_dir, epoch, identifier='model', device=0):
        ...
        """
        if device==0: device=self.device
        print("load_state using device choice:",device,flush=True)
...
class ConvBackbone(Backbone):
    ...
    def forward(self, batch, t):
        ...
        # logger.info(f"Input shape before interpolation: {batch.shape}")
        if self.interpolate:
            upsampled = self.interp.to_target(batch)
            # logger.info(f"Input shape after interpolation: {upsampled.shape}")
        else:
            upsampled = batch

        self_condition = self.get_self_condition(upsampled, t)
        upsampled_out = self.model(
            x=upsampled.to(self.device), time=t.to(self.device), x_self_cond=self_condition
        )
        # logger.info(f"Output shape before interpolation: {upsampled_out.shape}")

        if self.interpolate:
            batch_out = self.interp.from_target(upsampled_out.to("cpu"))
            # logger.info(f"Output shape before interpolation: {batch_out.shape}")
        else:
